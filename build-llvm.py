...
class EnvVars:
    def __init__(self, cc, cxx, ld, ar, llvm_tblgen, ranlib, clang_tblgen):
        self.cc = cc
        self.cxx = cxx
        self.ld = ld
        self.ar = ar
        self.llvm_tblgen = llvm_tblgen
        self.ranlib = ranlib
        self.clang_tblgen = clang_tblgen
...
def check_cc_ld_variables(root_folder):
    ...
    possible = versioned_binaries("llvm-ar")
    for compiler in possible:
        ar = shutil.which(compiler)
        if ar is not None:
            break
    possible = versioned_binaries("llvm-tblgen")
    for compiler in possible:
        llvm_tblgen = shutil.which(compiler)
        if llvm_tblgen is not None:
            break
    possible = versioned_binaries("llvm-ranlib")
    for compiler in possible:
        ranlib = shutil.which(compiler)
        if ranlib is not None:
            break
    possible = versioned_binaries("clang-tblgen")
    for compiler in possible:
        clang_tblgen = shutil.which(compiler)
        if clang_tblgen is not None:
            break

    return cc, cxx, ld, ar, llvm_tblgen, ranlib, clang_tblgen
...
def get_final_stage(args):
    ...
    if args.pgo and not args.build_stage1_only:
        total = 3
    elif not args.pgo and args.build_stage1_only:
        total = 1
...
def cc_ld_cmake_defines(args, dirs, env_vars, stage):
    ...
    if stage == 1 or (args.pgo and args.build_stage1_only):
        ar = env_vars.ar
        cc = env_vars.cc
        clang_tblgen = env_vars.clang_tblgen
        cxx = env_vars.cxx
        ld = env_vars.ld
        llvm_tblgen = env_vars.llvm_tblgen
...
def stage_specific_cmake_defines(args, dirs, stage):
    ...
    if bootstrap_stage(args, stage):
        ...
        # Build with instrumentation if we are on PGO instrumented stage
        if instrumented_stage(args, stage):
...
def invoke_ninja(args, dirs, stage):
    ...
    elif stage == get_final_stage(args) and args.build_stage1_only and not args.install_stage1_only:
...
def generate_pgo_profiles(args, dirs):
    ...
    if args.pgo and args.build_stage1_only:
        possible = versioned_binaries("llvm-profdata")
        for compiler in possible:
            profdata = shutil.which(compiler)
            if profdata is not None:
                break
        if profdata is None:
            raise RuntimeError(
                "llvm-profdata could be found on your system!")
    ...
    # Combine profiles
    if profdata is not None:
        subprocess.run([profdata, "merge",
            "-output=%s" % dirs.build_folder.joinpath("profdata.prof").as_posix()
        ] + glob.glob(
            dirs.build_folder.joinpath("stage1", "profiles",
                                       "*.profraw").as_posix()),
                       check=True)
    else:
        subprocess.run([
...
def do_multistage_build(args, dirs, env_vars):
    ...
    while stage <= get_final_stage(args):
        ...
        # Build profiles for PGO instrumented stage
        if instrumented_stage(args, stage):
